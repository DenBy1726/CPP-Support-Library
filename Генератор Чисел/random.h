...
float CRand::getFloat(int min , int max ,int number_of_sign)
{
	...
	max = (max == 0 ? mod - 1 : max);
	float dot = 0;
	for (int i = 0; i < number_of_sign; i++)
	{
		x_cur = get();
		dot += float(x_cur%10) / float(pow(10, i + 1));
	}
	x_cur = get();
	MaxGen mantiss = min + (x_cur/10) % (max - min);
	float res = (float)mantiss + (float)dot;
	return res;
}
...
{
	float mantiss = float(get());
	for (int i = 0; i < sign_num; i++)
		mantiss += float((rand10() / pow(10, i + 1)));
	return mantiss;

}
float Rand::getFloat(long min, long max, long sign_num)
{
	float mantiss = float(min + get()%(max+min));
	for (int i = 0; i < sign_num; i++)
	{
		mantiss += (rand10() / float(pow(10, i+1)));
	}
	return mantiss;
}
...
void Rand::setTime()
{
	_time = (unsigned long)::time(NULL);
	srand(_time);
}
bool Rand::getBit()
{
	return (randBit() == 0 ? true : false);
}
