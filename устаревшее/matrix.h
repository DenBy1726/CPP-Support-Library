...
public:
	...
	std::vector<_Type> operator[](size_t i)const
	{
		return _elem[i];
	}
};
...
*/
	...
	{
	//	template <class KeyT,class std::pair<int,int>> friend class AvlTreeBase;
	...
	public:
		...
		AssociativeMatrix(const AssociativeMatrix& x)
		{
			cur = x.cur;
			max = x.max;
			_elem = x._elem;
			//data.root = x.data._copy(x.data.root, x.data.root);
			data = x.data;
		...
		std::pair<int, int> find(KeyT k) const
		{
			return data.find(k);
		}
	};
